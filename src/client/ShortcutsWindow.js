...
const htmlPath = path.join(__dirname, '../public/shortcuts.html');
const configPath = path.join(__dirname, '../../shortcutsWindowConfig.json');

/**
 * A manager class to handle the shortcuts configuration window,
...
class ShortcutsWindow {
    ...
    _loadConfig() {
        try {
            ...
            console.error('Failed to read shortcuts window config, using defaults.', error);
        }
    ...
    _saveConfig() {
        ...
        try {
            ...
            console.error('Failed to save shortcuts window config.', error);
        }
    }

    /**
     * Creates and displays the shortcuts configuration window.
    ...
    create() {
        ...
        this._window = new BrowserWindow({
            ...
            transparent: true,
            frame: false,
            title: 'BPSR-PSO Shortcuts Configuration',
            icon: iconPath,
            alwaysOnTop: true,
            resizable: true,
            webPreferences: {
    ...
    /**
     * Loads a URL into the shortcuts window.
     */
    loadURL(url) {
        if (this._window && !this._window.isDestroyed()) {
